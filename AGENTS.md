...
Мод для Factorio 2.0 (`spidertrontiers-sa`), добавляющий 13 вариантов спайдертронов. Чистый Lua — без сборки, тестов и линтера.
...
4. `data-final-fixes.lua` — синхронизация категорий оборудования во всех гридах
...
Все пути require используют префикс `__spidertrontiers-sa__`. Совпадает с `"name"` в `info.json`. При форке и переименовании нужно обновить все `require()` и `filename`.
